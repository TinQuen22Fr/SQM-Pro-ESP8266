...
// -----------------------------------------------------------------------------
// Time-throttled battery cache (v2.2.16)
// -----------------------------------------------------------------------------
// The main loop runs at a few Hz, which made the OLED battery display flicker
// between successive readings (especially due to high-source-impedance ADC
// noise on the 100k+100k divider, even with oversampling). To stabilize the
// display we only re-sample the battery every BATTERY_DISPLAY_INTERVAL_MS
// (default 5 s). Callers can use:
//
...
static float _bat_cached_vbat   = -1.0f;   // -1 = not initialized yet
static byte  _bat_cached_pcent  = 255;     // 255 = not initialized yet
...
bool batteryRefreshIfDue() {
  ...
  bool millisRolled = (now < _bat_last_update_ms);  // handle millis() overflow
  if (firstCall || intervalElapsed || millisRolled) {
    // Single oversampled read, used for raw / V / %
    long sum = 0;
    for (int i = 0; i < BATTERY_OVERSAMPLES; i++) {
      sum += analogRead(A0);
      delay(2);
...
void DisplWait(char blk) {
  ...
  // -------------------------------------------------------------------------
  // Battery readout (calibrated, oversampled, time-cached since v2.2.16)
  // Format: "Bat: X.XXV (YY%)" or warning/critical icon if low.
  //
  // We use a time-throttled cache (BATTERY_DISPLAY_INTERVAL_MS, default 5 s)
  // so that successive calls to DisplWait() within the same interval reuse
  // the previous reading. This stabilizes the OLED display and reduces
  // ADC noise visibility on the high-impedance 100k+100k divider.
  //
  // The [BAT] serial trace is only emitted when the cache is actually
  // refreshed (= once every ~5 s), in WiFi mode only (USB mode reserves
  // Serial for the Unihedron protocol -- fixed in v2.2.9).
  // -------------------------------------------------------------------------
