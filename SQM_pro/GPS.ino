...
void ProcGPSData() {
  ...
  Serial.print(" SAT: "); Serial.print(g_sat);
  Serial.print(" LAT: "); Serial.print(g_lat);
  Serial.print(" LNG: "); Serial.print(g_lng);
  Serial.print(" ALT: "); Serial.println(g_alt);
  sprintf(_tmp, "%02d/%02d/%02d", g_day, g_month, g_year);
  Serial.print(" Date: "); Serial.println(_tmp);
  sprintf(_tmp, "%02d:%02d:%02d", g_hour, g_minute, g_second);
  Serial.print(" Time: "); Serial.println(_tmp);
#endif
} // end of ProcGPSData()

// -----------------------------------------------------------------------------
// Degree -> minutes/seconds helpers
// -----------------------------------------------------------------------------
...
int i_g_sec(double g_in) {
  return int(((g_in - int(g_in) * 1.) * 3600. + 0.5)
           - int((g_in - int(g_in) * 1.) * 60.) * 60 + .5);
}

int i_g_sec_s(double g_in) {
  return (((g_in - int(g_in) * 1.) * 3600. + 0.5)
        - int((g_in - int(g_in) * 1.) * 60.) * 60);
}
