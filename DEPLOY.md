...
| **OLED affiche `GPS no wire!`**                 | 1️⃣ **Vérifier en premier les câbles GPS-NodeMCU** : c'est la cause #1. Convention UART : `GPS TXD → NodeMCU D4 (RX)` et `GPS RXD → NodeMCU D7 (TX)` (toujours croisé, TX↔RX). 2️⃣ Si OK, vérifier l'alim du GPS sur **3,3 V** (pas 5 V) et la LED rouge qui doit être allumée. |
